...
describe("Find all posts usecase", () => {
  ...
  beforeAll(() => {
    ...
    createPostUseCase = new CreatePostUseCase(inMemoryPostRepository, inMemoryUserRepository);
    findAllPostsUseCase = new FindAllPostsUseCase(inMemoryPostRepository);
  });

  it("Should be able to find all Posts", async () => {
    await expect(createPostUseCase.execute(createPostMock)).resolves.not.toThrow();
    ...
    await expect(findAllPostsUseCase.execute(1, 10)).resolves.not.toThrow();
